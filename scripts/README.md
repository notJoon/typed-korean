...
All tables are regenerated by running `npm run codegen`, which executes `scripts/gen-jamo-table.ts`. The generated files are committed to version control; the Unicode block and compose jongseong list in that script are their source of truth. Vocabulary changes do not require table changes.
